...
object Huffman {
   ...
   *   }
   */
    def times(chars: List[Char]): List[(Char, Int)] = chars.map(c => (c,1)).groupBy(_._1).
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith((p1,p2) => p1._2 < p2._2)
    ...
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def traverse(remaining : CodeTree, bits : List[Bit]) : List[Char] = {
        remaining match{
          ...
          case Fork(l,r,chars, w) if bits.head == 0 => traverse(l, bits.tail)
          case Fork(l,r,chars, w) => traverse(r, bits.tail)
        }
      }
      traverse(tree, bits)
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def lookup(subtree: CodeTree)(char : Char) : List[Bit] = {
        subtree match {
          case Leaf(_, _) => List()
      ...
      text.map(c => lookup(tree)(c)).flatMap(f => f)
    }
