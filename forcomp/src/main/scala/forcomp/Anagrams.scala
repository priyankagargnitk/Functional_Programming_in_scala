...
object Anagrams {
  ...
  def wordAnagrams(word: Word): List[Word] = dictionaryByOccurrences getOrElse (wordOccurrences(word), null)
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {(y foldLeft SortedMap[Char,Int]() ++ x){ case (map, (ch, tm)) => {
    val newTm = map(ch) - tm
   ...
   */
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = { def iter(occurrences: Occurrences): List[Sentence] = {
